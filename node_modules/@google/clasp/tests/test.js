...
var TEST_APPSSCRIPT = JSON.stringify({ timeZone: 'America/New_York' });
...
var SCRIPT_ID = process.env.SCRIPT_ID || '';
...
mocha_1.describe('Test clasp clone <scriptId> function', function () {
    ...
    mocha_1.it('should clone a project with scriptId correctly', function () {
        cleanup();
        var result = spawnSync(CLASP, ['clone', SCRIPT_ID], { encoding: 'utf8' });
    ...
    mocha_1.it('should clone a project with scriptURL correctly', function () {
        cleanup();
        var result = spawnSync(CLASP, ['clone', utils_js_1.URL.SCRIPT(SCRIPT_ID)], { encoding: 'utf8' });
        chai_1.expect(result.stdout).to.contain('Cloned');
        chai_1.expect(result.stdout).to.contain('files.');
        chai_1.expect(result.status).to.equal(0);
    });
...
mocha_1.describe('Test clasp open function', function () {
    ...
    mocha_1.it('should prompt for which deployment to open correctly', function () {
        ...
        chai_1.expect(result.stdout).to.contain("Opening script: " + utils_js_1.URL.SCRIPT(SCRIPT_ID));
    });
    after(cleanup);
});
mocha_1.describe('Test URL utils function', function () {
    mocha_1.it('should create Script URL correctly', function () {
        var expectedUrl = "https://script.google.com/d/" + SCRIPT_ID + "/edit";
        chai_1.expect(utils_js_1.URL.SCRIPT(SCRIPT_ID)).to.equal(expectedUrl);
    });
    mocha_1.it('should create Creds URL correctly', function () {
        var expectedURL = "https://console.developers.google.com/apis/credentials?project=" + SCRIPT_ID;
        chai_1.expect(utils_js_1.URL.CREDS(SCRIPT_ID)).to.equal(expectedURL);
    });
});
...
mocha_1.describe('Test clasp version and versions function', function () {
    ...
    mocha_1.it('should create a new version correctly', function () {
        var result = spawnSync(CLASP, ['version', 'xxx'], { encoding: 'utf8' });
        versionNumber =
...
mocha_1.describe('Test clasp clone function', function () {
    ...
    mocha_1.it('should prompt which project to clone and clone it', function () {
        cleanup();
        var result = spawnSync(CLASP, ['clone'], { encoding: 'utf8', input: '\n' });
        chai_1.expect(result.stdout).to.contain('Clone which script?');
        chai_1.expect(result.stdout).to.contain('Cloned');
        chai_1.expect(result.stdout).to.contain('files.');
        chai_1.expect(result.status).to.equal(0);
...
mocha_1.describe('Test setting function', function () {
    ...
    mocha_1.it('should return current setting value', function () {
        var result = spawnSync(CLASP, ['setting', 'scriptId'], { encoding: 'utf8' });
    ...
    mocha_1.it('should update .clasp.json with provided value', function () {
        var result = spawnSync(CLASP, ['setting', 'scriptId', 'test'], { encoding: 'utf8' });
    ...
    mocha_1.it('should error on unknown keys', function () {
        // Test getting
        var result = spawnSync(CLASP, ['setting', 'foo'], { encoding: 'utf8' });
        chai_1.expect(result.status).to.equal(1);
        chai_1.expect(result.stderr).to.contain('Unknown key "foo"');
        // Test setting
        result = spawnSync(CLASP, ['setting', 'bar', 'foo'], { encoding: 'utf8' });
        chai_1.expect(result.status).to.equal(1);
        chai_1.expect(result.stderr).to.contain('Setting "bar" is unsupported');
    });
    after(cleanup);
});
...
mocha_1.describe('Test clasp apis functions', function () {
    ...
    mocha_1.it('should enable sheets', function () {
        var result = spawnSync(CLASP, ['apis', 'enable', 'sheets'], { encoding: 'utf8' });
        chai_1.expect(result.status).to.equal(0);
        chai_1.expect(result.stdout).to.contain('Enabled sheets.');
    });
    mocha_1.it('should give error message for non-existent API', function () {
        var result = spawnSync(CLASP, ['apis', 'enable', 'fakeApi'], { encoding: 'utf8' });
        chai_1.expect(result.status).to.equal(1);
    ...
    mocha_1.it('should disable apis correctly', function () {
        var result = spawnSync(CLASP, ['apis', 'disable', 'sheets'], { encoding: 'utf8' });
        chai_1.expect(result.status).to.equal(0);
        chai_1.expect(result.stdout).to.contain('Disabled sheets.');
    });
